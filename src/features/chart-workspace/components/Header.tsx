...
const HeikinAshiIcon: React.FC<{ className?: string }> = ({ className = 'w-3.5 h-3.5' }) => (
  <svg
    viewBox="0 0 16 16"
    className={className}
    fill="currentColor"
  ...
  >
    {/* Up / Filled Candle */}
    <rect x="2.5" y="4.5" width="3" height="7" rx="0.5" />
    <line x1="4" y1="2" x2="4" y2="4.5" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" />
    <line x1="4" y1="11.5" x2="4" y2="14" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" />
    {/* Down / Hollow Candle */}
    <rect x="10.5" y="3.5" width="3" height="6.5" rx="0.5" fill="none" stroke="currentColor" strokeWidth="1.2" />
    <line x1="12" y1="1.5" x2="12" y2="3.5" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" />
    <line x1="12" y1="10" x2="12" y2="14.5" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" />
  </svg>
);

const CHART_TYPE_OPTIONS = [
  { value: 'line' as const, label: 'Line', icon: LineChart },
  { value: 'candlestick' as const, label: 'Candlesticks', icon: CandlestickChart },
  { value: 'heikin_ashi' as const, label: 'Heikin Ashi', icon: HeikinAshiIcon },
];
...
export const Header: React.FC<HeaderProps> = (props) => {
  ...
  return (
    <header className="h-12 bg-surface border-b border-border-def px-4 flex items-center justify-between select-none z-30">
      ...
      {/* Center: Timeframes & Chart Type Controls */}
      <div className="flex items-center gap-2">
        ...
        {/* Quick-Access Chart Type Control */}
        <div
          role="radiogroup"
          aria-label="Chart Type"
          className="flex items-center bg-app-bg p-0.5 rounded-lg border border-border-sub"
        >
          {CHART_TYPE_OPTIONS.map((opt) => {
            const isTypeActive = (chartType || 'candlestick') === opt.value;
            const Icon = opt.icon;
            return (
              <button
                key={opt.value}
                type="button"
                role="radio"
                aria-checked={isTypeActive}
                disabled={!hasData}
                onClick={() => onChartTypeChange(opt.value)}
                title={opt.label}
                aria-label={opt.label}
                className={`p-1.5 px-2 rounded-md transition-all flex items-center justify-center cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed ${
                  isTypeActive
                    ? 'bg-accent text-txt-inverse shadow-sm'
                    : 'text-txt-muted hover:text-txt-primary hover:bg-surface-hover'
                }`}
